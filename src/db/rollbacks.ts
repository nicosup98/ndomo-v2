...
const VALID_STATUSES: RollbackStatus[] = [
  "planned",
  "approved",
  "dry_run",
  "executing",
...
export function recordRollback(db: Database, input: InsertRollback): RollbackExecution {
  ...
  if (input.newDeploymentId) {
    ...
    if (!newDep)
      throw new Error(`ndomo: new_deployment '${input.newDeploymentId}' not found (FK violation)`);
  }
  // Status (default planned)
  const status: RollbackStatus = input.status ?? "planned";
  if (!VALID_STATUSES.includes(status)) {
    throw new Error(
      `ndomo: invalid rollback status '${status}' (must be one of: ${VALID_STATUSES.join(", ")})`,
    );
  }
...
export function listRollbacksForIncident(db: Database, incidentId: string): RollbackExecution[] {
  const rows = db
    .query("SELECT * FROM rollback_executions WHERE incident_id = ? ORDER BY created_at DESC")
    .all(incidentId);
  return rows.map(rollbackFromRow);
}

export function listRollbacksForDeployment(
  db: Database,
  deploymentId: string,
): RollbackExecution[] {
  const rows = db
    .query("SELECT * FROM rollback_executions WHERE deployment_id = ? ORDER BY created_at DESC")
    .all(deploymentId);
  return rows.map(rollbackFromRow);
}
