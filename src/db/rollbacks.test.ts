...
function createTestDeployment(db: Database, id = "d1"): string {
  const now = Date.now();
  db.query(
    "INSERT OR IGNORE INTO environments (id, name, slug, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
  ).run("e1", "prod", "prod", now, now);
  db.query(
    "INSERT OR IGNORE INTO releases (id, version, title, created_at) VALUES (?, ?, ?, ?)",
  ).run("r1", "1.0.0", "rel", now);
  db.query(
    "INSERT INTO deployments (id, release_id, environment_id, status, created_at) VALUES (?, ?, ?, ?, ?)",
  ).run(id, "r1", "e1", "planned", now);
  return id;
}
...
describe("rollbacks.ts", () => {
  ...
  describe("recordRollback", () => {
    ...
    test("throws on invalid status", () => {
      createTestDeployment(db);
      expect(() =>
        recordRollback(db, { deploymentId: "d1", plan: "Bad", status: "invalid" as any }),
      ).toThrow("invalid rollback status");
    });

    test("throws on empty plan (whitespace only)", () => {
      createTestDeployment(db);
      expect(() => recordRollback(db, { deploymentId: "d1", plan: "   " })).toThrow(
    ...
    test("FK validation — non-existent deployment_id throws", () => {
      expect(() => recordRollback(db, { deploymentId: "nonexistent", plan: "test" })).toThrow(
    ...
    test("FK validation — non-existent incident_id throws", () => {
      createTestDeployment(db);
      expect(() =>
        recordRollback(db, { deploymentId: "d1", plan: "test", incidentId: "nonexistent" }),
    ...
    test("FK validation — non-existent new_deployment_id throws", () => {
      createTestDeployment(db);
      expect(() =>
        recordRollback(db, { deploymentId: "d1", plan: "test", newDeploymentId: "nonexistent" }),
