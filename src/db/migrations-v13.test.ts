...
describe("migration v13 — ops tables (T2)", () => {
  ...
  test("all 5 ops tables exist", () => {
    ...
    const tables = db
      .query(
        "SELECT name FROM sqlite_master WHERE type='table' AND name IN ('environments','releases','deployments','incidents','rollback_executions') ORDER BY name",
      )
  ...
  test("idempotent — running migrations twice is a no-op", () => {
    ...
    const count = db.query("SELECT COUNT(*) as c FROM schema_version WHERE version=13").get() as {
      c: number;
    };
  ...
  test("incidents table has FK to deployments (triggered_by_deployment_id)", () => {
    runMigrations(db);
    const fks = db.query("PRAGMA foreign_key_list(incidents)").all() as Array<{
      table: string;
      from: string;
    }>;
  ...
  test("rollback_executions has FKs to deployments + incidents", () => {
    runMigrations(db);
    const fks = db.query("PRAGMA foreign_key_list(rollback_executions)").all() as Array<{
      table: string;
      from: string;
    }>;
    const tables = fks.map((f) => f.table);
  ...
  test("CHECK constraints enforce severity enum", () => {
    ...
    db.query(
      "INSERT INTO incidents (id, title, severity, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)",
    ).run("i1", "test", "sev1", "open", Date.now(), Date.now());
    // Invalid severity should fail
    expect(() => {
      db.query(
        "INSERT INTO incidents (id, title, severity, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)",
      ).run("i2", "test", "sev5", "open", Date.now(), Date.now());
    }).toThrow();
  });
  ...
  test("CHECK constraints enforce incident status enum", () => {
    ...
    expect(() => {
      db.query(
        "INSERT INTO incidents (id, title, severity, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)",
      ).run("i3", "test", "sev1", "invalid_status", Date.now(), Date.now());
    }).toThrow();
  });
  ...
  test("CHECK constraints enforce rollback status enum", () => {
    ...
    expect(() => {
      db.query(
        "INSERT INTO rollback_executions (id, deployment_id, status, plan, created_at) VALUES (?, ?, ?, ?, ?)",
      ).run("rb1", "d1", "invalid_status", "plan", Date.now());
    }).toThrow();
  });
  ...
  test("FK constraint — cannot insert deployment with non-existent release", () => {
    ...
    expect(() => {
      db.query(
        "INSERT INTO deployments (id, release_id, environment_id, status, created_at) VALUES (?, ?, ?, ?, ?)",
      ).run("d1", "nonexistent", "e1", "planned", Date.now());
    }).toThrow();
  });
