...
describe("incidents.ts", () => {
  ...
  describe("createIncident", () => {
    ...
    test("throws on invalid severity", () => {
      expect(() => createIncident(db, { title: "Bad", severity: "sev5" as any })).toThrow(
        "invalid incident severity",
      );
    });

    test("throws on empty title (whitespace only)", () => {
      expect(() => createIncident(db, { title: "   ", severity: "sev1" })).toThrow(
  ...
  describe("updateIncidentStatus", () => {
    ...
    test("throws on invalid transition: open→postmortem", () => {
      ...
      expect(() => updateIncidentStatus(db, inc.id, "postmortem")).toThrow(
    ...
    test("throws on backward transition: resolved→mitigated", () => {
      ...
      expect(() => updateIncidentStatus(db, inc.id, "mitigated")).toThrow(
        "invalid incident transition",
      );
    });
    ...
    test("throws on non-existent incident", () => {
      expect(() => updateIncidentStatus(db, "nonexistent", "triaging")).toThrow(
        "incident 'nonexistent' not found",
      );
    });
    ...
    test("postmortem is terminal — no further transitions", () => {
      ...
      expect(() => updateIncidentStatus(db, inc.id, "resolved")).toThrow(
        "invalid incident transition",
      );
    });
  });
