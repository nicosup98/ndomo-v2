...
export function createIncident(db: Database, input: InsertIncident): Incident {
  ...
  if (input.triggeredByDeploymentId) {
    const dep = db
      ...
      .get(input.triggeredByDeploymentId);
    if (!dep)
      throw new Error(
        `ndomo: deployment '${input.triggeredByDeploymentId}' not found (FK violation)`,
      );
  }
  ...
  db.query(
    ...
    input.severity,
    summary,
    input.triggeredByDeploymentId ?? null,
    now,
    now,
    metadata,
  );
...
export function listIncidents(
  ...
  params.push(limit);
  const rows = db
    .query(`SELECT * FROM incidents ${where} ORDER BY created_at DESC LIMIT ?`)
    .all(...params);
...
export function updateIncidentStatus(
  ...
  if (!allowed.includes(newStatus)) {
    throw new Error(
      `ndomo: invalid incident transition '${currentStatus}' → '${newStatus}' (allowed: ${allowed.join(", ") || "none (terminal)"})`,
    );
  }
  const now = Date.now();
  const resolvedAt =
    newStatus === "resolved" || newStatus === "postmortem" ? now : current.resolvedAt;
  db.query("UPDATE incidents SET status = ?, updated_at = ?, resolved_at = ? WHERE id = ?").run(
    newStatus,
    now,
    resolvedAt,
    id,
