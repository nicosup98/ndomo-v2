...
describe("migration v16 — plans.owner (ADR-010)", () => {
  ...
  test("plans.owner column exists with default 'foreman'", () => {
    runMigrations(db);
    const cols = db.query("PRAGMA table_info(plans)").all() as Array<{
      name: string;
  ...
  test("idempotent — running migrations twice is a no-op", () => {
    ...
    const count = db.query("SELECT COUNT(*) as c FROM schema_version WHERE version=16").get() as {
      c: number;
    };
  ...
  test("plans.owner accepts valid enum values", () => {
    ...
    const now = Date.now();
    db.query(
      "INSERT INTO plans (id, slug, title, status, priority, owner, overview, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
    ...
    const rows = db.query("SELECT id, owner FROM plans ORDER BY id").all() as Array<{
      id: string;
      owner: string;
    }>;
  ...
  test("existing plans get default 'foreman' after migration", () => {
    ...
    const row = db.query("SELECT owner FROM plans WHERE id=?").get("p_default") as {
      owner: string;
    };
